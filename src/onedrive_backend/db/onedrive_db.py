...
def insert_credentials_to_db(Input):
    ...
    cursor.execute(
        """
        INSERT INTO connector1 (connector_id, type, app_id, config, created_date, updated_date, token)
...
def get_connector_by_email(client_id):
    conn = get_connection()
    cursor = conn.cursor()

    cursor.execute(
        "SELECT connector_id, config FROM connector1 WHERE config ->> 'client_id' = %s",
        (client_id,)
    )
    row = cursor.fetchone()
    if not row:
...
def insert_log_entry(connector_id, app_id, document_name, additional_info, status, message_id=None):
    try:
        ...
        if cur.rowcount > 0:
            ...
            print(f"Skipped duplicate log for message_id={message_id}")
...
def get_connector_by_email_by_client_id(client_id):
    ...
    cursor.execute(
    "SELECT connector_id, config, token, app_id FROM connector1 WHERE config ->> 'client_id' = %s",
    (client_id,)
    )
...
def message_already_processed(message_id: str) -> bool:
    # Allow duplicates unless exact match exists
    try:
        conn = get_connection()
        cur = conn.cursor()
        cur.execute("SELECT 1 FROM connector_log1 WHERE message_id = %s", (message_id,))
...
def does_it_exist(client_id: str) -> bool:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT 1 FROM connector1 WHERE config ->> 'client_id' = %s",
        (client_id,)
    )
